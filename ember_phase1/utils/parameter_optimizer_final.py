...
def ensure_attention_compatibility(embed_dim: int) -> int:
    """
    Ensure embed_dim works with the attention mechanism in LModule
    The LModule calculates num_heads as max(4, embed_dim // 128)
    So embed_dim must be divisible by this calculated number of heads
    """
    calculated_heads = max(4, embed_dim // 128)
    
    # If embed_dim is not divisible by calculated_heads, adjust it
    if embed_dim % calculated_heads != 0:
        # Round embed_dim to the nearest value that is divisible by calculated_heads
        rounded = (embed_dim // calculated_heads) * calculated_heads
        # If this is too different from original, try adjusting the divisor
        if abs(embed_dim - rounded) > embed_dim * 0.05:  # If more than 5% difference
            # Find closest value divisible by calculated_heads that's close to embed_dim
            lower = (embed_dim // calculated_heads) * calculated_heads
            upper = lower + calculated_heads
            if abs(embed_dim - upper) < abs(embed_dim - lower):
                embed_dim = upper
            else:
                embed_dim = lower
        else:
...
def create_model_with_target_params(target_params: int = 100_000_000, 
    ...
    base_embed_dim = ensure_attention_compatibility(base_embed_dim)
    ...
    def calculate_params(cfg: LModuleConfig) -> int:
        ...
        try:
            ...
            return float('inf')  # Return a large number to indicate this config doesn't work
    ...
    while (abs(current_params - target_params) > target_params * tolerance and 
           attempts < max_attempts and 
           current_params != float('inf')):
        ...
        if param_ratio > 1.1:  # Need more parameters
            ...
            new_embed_dim = ensure_attention_compatibility(new_embed_dim)
        ...
        config.embed_dim = max(512, min(2048, config.embed_dim))
        config.embed_dim = ensure_attention_compatibility(config.embed_dim)
